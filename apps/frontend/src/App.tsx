...
function App() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      ...
      try {
        const fileTypeFilter = FILE_TYPE_FILTERS.find(
          (filter) => filter.id === fileTypeFilterId,
        );
        const sortOption =
          SORT_OPTIONS.find((option) => option.id === sortOptionId) ??
          SORT_OPTIONS[0];
        ...
        const response = await fetch(
          `${BACKEND_URL}/api/search?${params.toString()}`,
          {
        ...
        setError(
          err instanceof Error ? err.message : "Unable to fetch results.",
        );
        setResults([]);
      } finally {
        setIsLoading(false);
  ...
  const handleRefreshThumbnails = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/api/thumbnails`, {
        method: "DELETE",
      });
      ...
      setError(
        err instanceof Error
          ? err.message
          : "No se pudo limpiar la caché de miniaturas.",
      );
    } finally {
